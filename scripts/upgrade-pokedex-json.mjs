...
for (let i = 0; i < pokemonList.length; i++) {
    ...
    try {
      ...
      const twNameObj = speciesData.names.find(n => n.language.name === 'zh-hant') || 
...
await fs.writeFile('./server/api/data/pokedex.json', JSON.stringify(pokemonList, null, 2), 'utf-8');
